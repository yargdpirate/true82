...
const SHARE_BUILD = "2026-07-11.tribune-share-v2";
...
const SLUG_RE = /^[a-z0-9](?:[a-z0-9-]{3,68}[a-z0-9])$/;
...
async function publish(request, env, id) {
  ...
  try {
    const result = await env.DB.prepare(
      ...
      JSON.stringify(canon.players), SHARE_BUILD
    ).run();
    return json({ ok: true, id, created: !!(result && result.meta && result.meta.changes) }, result && result.meta && result.meta.changes ? 201 : 200);
  } catch (e) {
...
function escJs(v) { return String(v == null ? "" : v).replace(/[^a-z0-9-]/gi, ""); }
