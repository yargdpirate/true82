...
export async function onRequest(context) {
  ...
  const [completes, roundFunnel, modeMix, hcAction, hcSeg, shareByU, deviceMix, referrers, endRows, donateMix] =
    await Promise.all([
      q(`SELECT mode, wins, undefeated, sid FROM events WHERE name='game_complete' LIMIT 100000`),
  ...
  const sessions   = cnt(await one(`SELECT COUNT(*) c FROM events WHERE name='session_start'`));
  const starts     = cnt(await one(`SELECT COUNT(*) c FROM events WHERE name='game_start'`));
  const startSids  = cnt(await one(`SELECT COUNT(DISTINCT sid) c FROM events WHERE name='game_start'`));
  const completesN = cnt(await one(`SELECT COUNT(*) c FROM events WHERE name='game_complete'`));
  const shown      = cnt(await one(`SELECT COUNT(*) c FROM events WHERE name='heatcheck_shown'`));
  const errors     = cnt(await one(`SELECT COUNT(*) c FROM events WHERE name='data_error'`));
  const loadRow    = await one(`SELECT ROUND(AVG(load_ms)) ms FROM events WHERE name='data_ready'`);
  const sessRow    = await one(`SELECT ROUND(AVG(duration)/1000.0,1) sec, ROUND(AVG(games_played),2) g, MAX(max_round) mr FROM events WHERE name='session_end'`);
  const prestiRow  = await one(`SELECT ROUND(AVG(budget_used),1) bu, ROUND(AVG(roster_value),1) rv, COUNT(*) c FROM events WHERE name='game_complete' AND mode='cap'`);
  ...
  cards.push(card("Heat Check", `
    ...
    ${hcSeg.length ? hcSeg.map((r) => bar(esc(r.segment), +r.c || 0, segMax, `${r.c}${(+r.h || 0) ? " · " + r.h : ""}`)).join("") : muted("no spins yet")}`));
  ...
  cards.push(card("Donate clicks", donateTotal ? `
    ...
    ${donateMix.map((r) => bar(esc(r.variant), +r.c || 0, donateMax)).join("")}`
  ...
  cards.push(card("Traffic & tech", `
    ...
    ${deviceMix.length ? deviceMix.map((r) => bar(esc(r.d), +r.c || 0, devMax)).join("") : muted("—")}
    <div class="sub">top referrers</div>
    ${referrers.length ? referrers.map((r) => bar(esc(shortRef(r.referrer)), +r.c || 0, refMax)).join("") : muted("(direct / none)")}
...
function bar(label, value, max, sub) {
  ...
  return `<div class="row"><span class="rl">${label}</span><span class="rt"><span class="fill" style="width:${w}%"></span></span><span class="rv">${esc(sub != null ? sub : value)}</span></div>`;
}
